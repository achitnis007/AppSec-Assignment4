...
_ = open(secretspath+'admin_password', 'r'); password = _.read().replace('\n', ''); _.close()
_ = open(secretspath+'admin_2fa', 'r'); phonenum = _.read().replace('\n', ''); _.close()
hashed_password = bcrypt.generate_password_hash(password).decode('utf-8')
user = User(username='admin', phone=phonenum, password=hashed_password)
db.session.add(user)

_ = open(secretspath+'actester1_password', 'r'); password = _.read().replace('\n', ''); _.close()
_ = open(secretspath+'actester1_2fa', 'r'); phonenum = _.read().replace('\n', ''); _.close()
hashed_password = bcrypt.generate_password_hash(password).decode('utf-8')
user = User(username='actester1', phone=phonenum, password=hashed_password)
db.session.add(user)

_ = open(secretspath+'actester2_password', 'r'); password = _.read().replace('\n', ''); _.close()
_ = open(secretspath+'actester2_2fa', 'r'); phonenum = _.read().replace('\n', ''); _.close()
hashed_password = bcrypt.generate_password_hash(password).decode('utf-8')
user = User(username='actester2', phone=phonenum, password=hashed_password)
db.session.add(user)

_ = open(secretspath+'actester3_password', 'r'); password = _.read().replace('\n', ''); _.close()
_ = open(secretspath+'actester3_2fa', 'r'); phonenum = _.read().replace('\n', ''); _.close()
hashed_password = bcrypt.generate_password_hash(password).decode('utf-8')
